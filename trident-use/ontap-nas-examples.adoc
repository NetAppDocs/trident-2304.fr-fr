...
[role="lead"]
Découvrez comment créer et utiliser des pilotes NAS ONTAP avec votre installation Astra Trident. Cette section présente des exemples de configuration du back-end et des détails sur le mappage des systèmes back-end aux classes de stockage.
...
Pour un basculement MetroCluster transparent, vous devez spécifier une LIF de gestion du SVM.
...
Peut être configuré pour utiliser des adresses IPv6 si Astra Trident a été installé à l'aide du `--use-ipv6` drapeau. Les adresses IPv6 doivent être définies entre crochets, telles que [28e8:d9fb:a825:b7bf:69a8:d02f:9e7b:3555]. | Adresse spécifiée ou dérivée d'un SVM, si non spécifiée (non recommandé) 
...
| `autoExportCIDRs` | Liste des CIDR pour filtrer les adresses IP du nœud Kubernetes par rapport à quand `autoExportPolicy` est activé.
...
| `snapshotReserve` | Pourcentage du volume réservé pour les snapshots « 0 » | Si `snapshotPolicy` est « aucun », sinon « » 
...
| `tieringPolicy` | Règle de Tiering à utiliser « aucun » | Snapshot uniquement pour une configuration SVM-DR pré-ONTAP 9.5 
...
.Configuration minimale pour <code>ontap-nas-economy</code>
[%collapsible%open]
====
[listing]
----
...
----
====
.Configuration minimale pour <code>ontap-nas-flexgroup</code>
[%collapsible%open]
====
[listing]
----
...
----
====
.Configuration minimale pour les volumes SMB
[%collapsible%open]
====
[listing]
----
...
----
====
.Amazon FSX pour ONTAP utilisant des volumes SMB
[%collapsible%open]
====
